...
async function fetchSeededQuestion(topic) {
    const { data, error } = await supabase
        .from("questions")
        .select("id, topic, difficulty, title, prompt, test_cases")
        .eq("topic", topic);
...
window.launchOA = async function(companyKey) {
    ...
    // Set to true only for local testing — forces every launch onto a
    // topic that has seeded questions, so real grading always triggers.
    const TESTING_FORCE_ARRAYS = false;

    const oaTopics = ["Arrays", "Strings", "Hashing", "Trees", "Graphs", "Dynamic Programming", "Greedy", "Binary Search", "Sliding Window"];
    currentOATopic = TESTING_FORCE_ARRAYS ? "Arrays" : oaTopics[Math.floor(Math.random() * oaTopics.length)];
    ...
    // Try a real, verified, gradeable question first (only covers topics
    // that have been seeded — currently just Arrays). If found, this is
    // the ONLY path that produces a real score in finishOA.
    const seeded = await fetchSeededQuestion(currentOATopic);
    if (seeded) {
        window.currentOAQuestionData = seeded;
    ...
    // No seeded question for this topic yet — fall back to the existing
    // live-generation path. NOTE: finishOA cannot grade these for real
    // (no stored test cases), so this remains practice-only/unscored.
    window.currentOAQuestionData = null;
    console.warn(`No seeded question for topic "${currentOATopic}" — falling back to unscored AI-generated practice problem.`);
...
window.finishOA = async function() {
    ...
    // Real grading path — only available when a verified seeded question
    // was loaded (see launchOA). Runs the submission against its actual
    // stored test cases via Piston.
    if (window.currentOAQuestionData) {
        try {
            const q = window.currentOAQuestionData;
            let testsPassed = 0;
            ...
            await saveSessionToSupabase({
                ...
                composites: null,
                topicScores: { [q.topic]: overallScore }
            }, 'oa');

            terminal.innerHTML += `<br><span style="color: var(--success);">[System] OA Submitted. Score: ${testsPassed}/${testsTotal} test cases passed (${overallScore}/100).</span>`;
            setTimeout(async () => {
                await refreshDashboardProfile();
                navigateTo('view-dashboard');
            }, 6000);
        } catch (err) {
            terminal.innerHTML += `<br><span style="color: var(--danger);">[System Error] Grading failed: ${err.message}. Is your local Piston container running? (Real grading only works when testing via http://localhost, not the live https site — see code comments.)</span>`;
        }
        return;
    }

    // Fallback path — no verified question was available for this topic,
    // so there are no real test cases to grade against. This score is NOT
    // real correctness verification; it's a rough heuristic for practice
    // problems only. Flagged clearly so this distinction isn't lost.
    console.warn("Using heuristic (non-verified) scoring — no stored test cases for this question.");
    ...
    try {
        ...
        terminal.innerHTML += `<br><span style="color: var(--warning);">[System] OA Submitted (practice mode, unscored problem). Estimated: ${overallScore}/100</span>`;
        setTimeout(async () => {
            await refreshDashboardProfile();
            navigateTo('view-dashboard');
        }, 6000);
...
function pickNextQuestion() {
    ...
    // Feature 5: Cross-Session Adaptive Framework
    ...
    const weights = pool.map(q => {
        const normalizedTopicScore = topicScores[q.topic] == null ? 0.5 : topicScores[q.topic] / 100;
        let weight = 1 / (normalizedTopicScore + 0.2); // Smooth to prevent starvation
        
        if (historicalWeakTopics.includes(q.topic)) {
            weight *= 2.0; // Boost probability of cross-session weaknesses safely
        }
        return weight;
    });
...
function startRecording() {
    ...
    window.speechSynthesis.cancel(); // Immediately silence any lingering AI speech
...
window._activeUtterance = null; // Fix for Chrome garbage collection bug
...
function speakInterviewerText(text, onEnd) {
    ...
    window._activeUtterance = utterance; // Keep a global reference so it isn't dropped
    let finished = false;
    const finish = () => {
    ...
    // Dynamic fallback timer based on text length (roughly 100ms per char + baseline)
    const fallbackTimer = setTimeout(finish, Math.max(text.length * 100, 10000));
    utterance.onend = () => {
